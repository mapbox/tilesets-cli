...
def test_validate_invalid_ldgeojson():
    runner = CliRunner()
    result = runner.invoke(cli, ['validate-source', filepath + '/fixtures/invalid.ldgeojson'])
...
def test_validate_invalid_geojson():
    runner = CliRunner()
    result = runner.invoke(cli, ['validate-source', filepath + '/fixtures/invalid-geojson.ldgeojson'])
    assert result.exit_code == 1
